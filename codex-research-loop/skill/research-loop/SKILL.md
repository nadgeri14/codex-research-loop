...
After Sol defines the intended behavior, file ownership, constraints, and validation criteria, delegate all bounded execution-ready implementation to Meta Muse Spark (`muse-spark-1.2-contributor` at `xhigh`) via the reviewed direct API worker `scripts/muse_research_worker.py`. Do not invoke raw `muse exec`. Muse Spark remains the sole implementation model; only the local Muse CLI orchestration layer is bypassed. Read [references/muse-spark.md](references/muse-spark.md) before invoking it.
