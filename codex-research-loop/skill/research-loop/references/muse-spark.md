...
- Nothing is applied automatically. Sol-high reviews the patch; a favorable review is followed by `python scripts/muse_cli_worker.py --root REPO --contract-id ID --owned-path REL --apply-patch EVIDENCE/patch.diff --check-json '["..."]'`, which re-verifies owned scope (tab-safe path parsing), requires the patch's recorded base `HEAD` to match the current `HEAD` (fail-closed; `--allow-base-mismatch` overrides), applies, then runs the declared checks in the main repository and rolls the patch back if any fails.
- Attempt ceilings, unchanged-failure suppression, and the repo lock mirror the direct worker; the repo lock is anchored at the canonical scratch root, so the two lanes exclude each other on one repository regardless of `--scratch-root` narrowing. Transient outcomes (interrupt, wall/validation timeout) neither suppress the identical retry nor consume ceilings. The main-repo contamination check runs on every post-run path, including timeouts and failures. Diffs are extracted against the base commit, so work an agent commits in the worktree is still captured. The worktree sees committed `HEAD` only — cut CLI-lane contracts against committed state.
- Portability: on another machine or account set `MUSE_WORKER_SCRATCH_ROOT` to relocate the canonical scratch root for both lanes (never `/tmp`/`/dev/shm`).
